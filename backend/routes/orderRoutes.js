...
router.post("/add", async (req, res) => {
  try {
    ...
    // Ensure `_id` is a string and images are valid
    items = items.map((item) => ({
      ...item,
      _id: item._id ? String(item._id) : new mongoose.Types.ObjectId().toString(),
      images: item.images && item.images.length > 0 ? item.images : ["http://localhost:5000/fallback-image.jpg"],
    }));
...
// Fetch All Orders (For User & Admin Panel)
...
// Update Order Status (for Users Tracking Orders)
...
// GET single order by ID (for tracking status)
